...
class EvalCallback():
    def __init__(self, net, input_shape, anchors_mask, class_names, num_classes, val_lines, log_dir, cuda, \
    ...
    #   将目标检测的结果输出到文件
    def get_map_txt(self, image_id, image, class_names, map_out_path, anchors):
        bbox_util = DecodeBox(anchors, self.num_classes, (self.input_shape[0], self.input_shape[1]),
                                   self.anchors_mask)
        ...
        with torch.no_grad():
            ...
            #   将预测框宽高拼接，然后进行非极大抑制
            results = bbox_util.non_max_suppression(torch.cat(outputs, 1), self.num_classes, self.input_shape,
    ...
    def on_epoch_end(self, epoch, model_eval, anchors):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for annotation_line in tqdm(self.val_lines):
                ...
                #   获得预测结果txt
                self.get_map_txt(image_id, image, self.class_names, self.map_out_path, anchors)
