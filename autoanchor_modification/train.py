...
if __name__ == "__main__":
    ...
    fp16 = False
    #   classes_path代表存储目标种类名字的txt文件
    classes_path = '/content/drive/MyDrive/yolo3_football/model_data/football_classes.txt'
    #   anchors_path代表存储先验框尺寸的txt文件
    #   anchors_mask用于帮助代码找到对应的先验框
    anchors_path = '/content/drive/MyDrive/yolo3_football/model_data/yolo_anchors.txt'
    anchors_mask = [[6, 7, 8], [3, 4, 5], [0, 1, 2]]
    #   预训练权重对不同数据集是通用的，因为特征是通用的。
    ...
    #   如果想要让模型从0开始训练，则设置model_path = ''，下面的pretrain = False，Freeze_Train = False，此时从0开始训练，且没有冻结主干的过程。
    #
    #   一般来讲，网络从0开始的训练效果会很差，因为权值太过随机，特征提取效果不明显，因此非常、非常、非常不建议大家从0开始训练！
    #   如果一定要从0开始，可以了解imagenet数据集，首先训练分类模型，获得网络的主干部分权值，分类模型的 主干部分 和该模型通用，基于此进行训练。
    model_path = '/content/drive/MyDrive/yolo3_football/model_data/yolo_weights.pth'
    ...
    #   获得图片路径和标签
    train_annotation_path = '/content/drive/MyDrive/yolo3_football/football_train.txt'
    val_annotation_path = '/content/drive/MyDrive/yolo3_football/football_valid.txt'
    ...
    #   获得损失函数
    yolo_loss = YOLOLoss(num_classes, input_shape, Cuda, anchors_mask)

    #   记录Loss用的loss_history
    if local_rank == 0:
    ...
    if local_rank == 0:
        ...
        #   总训练步长step指的是梯度下降的总次数
        #   每个训练世代包含若干训练步长，每个训练步长进行一次梯度下降。
        #   此处仅建议最低训练世代，上不封顶，计算时只考虑了解冻部分
        #   如果总训练步长太小，需要增加epoch值
    ...
    #   主干特征提取网络通用特征，冻结训练可以加快训练速度，也可以在训练初期防止权值被破坏。
    #   Init_Epoch为起始世代
    ...
    if True:
        ...
        #   根据当前batch_size、优化器类型自适应调整学习率
        maxbs = 64
        lr_limit_max = 1e-3 if optimizer_type == 'adam' else 5e-2
        lr_limit_min = 3e-4 if optimizer_type == 'adam' else 5e-4
        Init_lr_fit = min(max(batch_size / maxbs * Init_lr, lr_limit_min), lr_limit_max)
        Min_lr_fit = min(max(batch_size / maxbs * Min_lr, lr_limit_min * 1e-2), lr_limit_max * 1e-2)

        #   根据神经网络层的名字和参数类型(weight、bias)将参数分到3个list中
        pg0, pg1, pg2 = [], [], []
        ...
        #   根据optimizer_type选择优化器并将参数加入优化器
        optimizer = {
        ...
        #   加载数据
        #   包括图像、真实框数据两部分，尺寸都是batch_size, 图像/真实框原有维度
        ...
        #   记录eval的map曲线
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, anchors_mask, class_names, num_classes, val_lines,
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            #   如果模型有冻结学习部分,且世代数超过Freeze_Epoch，则解冻，并更新batch_size参数
            ...
            fit_one_epoch(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step,
                          epoch_step_val, gen, gen_val, input_shape, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir,
                          local_rank)
